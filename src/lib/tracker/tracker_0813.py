...
class JDETracker(object):
    ...
    def update(self, im_blob, img0):
        ...
        trust = - np.log(dets[:, 4])
        # dets_alpha = 0.95 + 0.05 * trust
        dets_alpha = 0.85 + 0.15 * trust
        dets_alpha = np.clip(dets_alpha, 0.85, 0.90)
        # vis
        '''
        ...
        for itracked, idet in matches:
            ...
            if track.state == TrackState.Tracked:
                track.update(detections[idet], self.frame_id, alpha=dets_alpha[idet])
                activated_starcks.append(track)
            else:
                track.re_activate(det, self.frame_id, new_id=False, alpha=dets_alpha[idet])
        ...
        for itracked, idet in matches:
            ...
            det = detections[idet]
            if track.state == TrackState.Tracked:
                track.update(det, self.frame_id, alpha=0.90)
                activated_starcks.append(track)
            else:
                track.re_activate(det, self.frame_id, new_id=False, alpha=0.90)
                refind_stracks.append(track)
        ...
        for itracked, idet in matches:
            ...
            if track.state == TrackState.Tracked:
                track.update(det, self.frame_id, alpha=0.90)  # 这里对低置信度值进行特征更新 不太合理
                activated_starcks.append(track)
            else:
                track.re_activate(det, self.frame_id, new_id=False, alpha=0.90)
                refind_stracks.append(track)
